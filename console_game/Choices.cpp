...
int Choices::choices_loop(short vertical_margin, Renderer& renderer)
{
	...
	std::string message = "ARROW KEY: MOVE   ENTER KEY: SELECT";
	...
	while (!is_selected)
	{
		...
		if (key_detector.get_curr_press().test(0) && !key_detector.get_prev_press().test(0))
			curr_cursor = curr_cursor - 1 < 0 ? choices_num - 1 : curr_cursor - 1;
		else if (key_detector.get_curr_press().test(2) && !key_detector.get_prev_press().test(2))
			curr_cursor = (curr_cursor + 1) % choices_num;
		else if (key_detector.get_curr_press().test(4) && !key_detector.get_prev_press().test(4))
			is_selected = true;
